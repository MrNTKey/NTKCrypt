...
import com.mrntkey.ntkcrypt.util.NTKCryptNotification
...
class KeepAliveService : Service() {
    ...
    companion object {
        private const val TAG = NTKCryptApp.TAG
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        NTKCryptNotification.createChannel(this)

        // 2. 创建一个通知
        val notification = NTKCryptNotification.build(this)

        // 3. ✨ 最关键的一步：将服务推到前台！
        //    第一个参数是一个唯一的通知ID，第二个参数是我们创建的通知。
        startForeground(NTKCryptNotification.NEKO_NOTIFICATION_ID, notification)
        // 我们同时创一个保活悬浮窗
